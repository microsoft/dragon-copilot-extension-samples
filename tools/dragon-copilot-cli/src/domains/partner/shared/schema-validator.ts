...
function getSchemaPath(): string {
  ...
  for (const candidate of candidates) {
    if (existsSync(join(candidate, 'partner-manifest.json'))) {
      return candidate;
    }
  }
...
function validateManifestBusinessRules(manifest: PartnerIntegrationManifest): SchemaError[] {
...
export function validateFieldValue(value: unknown, fieldPath: string, schemaType: 'manifest' | 'publisher'): string | true {
  const schema = schemaType === 'manifest' ? partnerManifestSchema : publisherSchema;
