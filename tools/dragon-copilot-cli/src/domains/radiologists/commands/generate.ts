...
export async function generateManifest(options: GenerateOptions): Promise<void> {
  console.log(chalk.blue('🐉 Generating Dragon Copilot Radiologists Extension Manifest'));
...
async function generateInteractive(options: GenerateOptions): Promise<void> {
  ...
  try {
    ...
    if (validation.isValid) {
      ...
      console.log(chalk.yellow('⚠️  Existing manifest is not a valid DCR manifest, creating new one'));
    }
  } catch {
