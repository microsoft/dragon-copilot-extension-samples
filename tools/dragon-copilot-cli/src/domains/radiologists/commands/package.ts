...
export async function packageExtension(options: PackageOptions): Promise<void> {
  const isQuiet = options.silent || process.env.NODE_ENV === 'test';
    ...
    logMessage(chalk.blue('🐉 Packaging Dragon Copilot Radiologists Extension'), isQuiet);
  ...
  if (!(await pathExists(manifestPath))) {
    ...
    logMessage(chalk.gray('   Create one using: dragon-copilot radiologists init'), isQuiet);
  ...
  try {
      logMessage(chalk.blue('\n📋 Validating radiologists extension manifest...'), isQuiet);
    ...
    if (!manifestValidation.isValid) {
        logMessage(chalk.red('❌ Radiologists extension manifest validation failed:'), isQuiet);
      ...
      logMessage(chalk.green('✅ Radiologists extension manifest is valid'), isQuiet);
    ...
    output.on('close', () => {
      ...
      logMessage(chalk.gray(`  • Radiologists extension manifest (extension.yaml)`), isQuiet);
