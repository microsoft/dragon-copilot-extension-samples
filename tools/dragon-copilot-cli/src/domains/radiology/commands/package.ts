...
export async function packageExtension(options: PackageOptions): Promise<void> {
  ...
  try {
    ...
    logMessage(chalk.gray(`???  Tools: ${manifest.tools?.length || 0}`), isQuiet);
    ...
    archive.on('warning', (err) => {
      if (err.code === 'ENOENT') {
        logMessage(chalk.yellow(`??  Warning: ${err.message}`), isQuiet);
      } else {
        throw err;
      }
    });
    ...
    if (options.include && options.include.length > 0) {
      ...
      for (const pattern of options.include) {
        if (await pathExists(pattern)) {
          ...
          logMessage(chalk.yellow(`??  File not found, skipping: ${pattern}`), isQuiet);
        }
      }
    }
