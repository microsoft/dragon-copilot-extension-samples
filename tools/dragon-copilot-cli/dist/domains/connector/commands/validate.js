...
export async function runValidateCommand(filePath) {
    ...
    if (!targetPath) {
        ...
        console.log(chalk.gray('  • `dragon-copilot connector validate` is run without a file path.'));
    ...
    if (!existsSync(resolvedPath)) {
        ...
        console.log(chalk.gray('Ensure the manifest exists or generate one with `dragon-copilot connector init`.'));
        process.exit(1);
    }
...
export async function validateManifest(filePath) {
    console.log(chalk.blue('🤝 Validating Clinical Application Connector Manifest'));
    ...
    try {
        ...
        const manifest = load(fileContent);
        ...
        const validateGuid = (value, field) => {
        ...
        const checkYesNo = (value, field, required = false) => {
        ...
        const checkFieldDefinition = (field, prefix, options = {}) => {
        ...
        const validateContextItems = (rawSection) => {
            ...
            const rawItems = rawSection.instance;
        ...
        if (!Array.isArray(serverAuthentication) || serverAuthentication.length === 0) {
            ...
            serverAuthentication.forEach((entry, index) => {
                ...
                if (!Array.isArray(entry?.identity_value) || entry.identity_value.length === 0) {
                    ...
                    entry.identity_value.forEach((value, valueIndex) => {
                        if (typeof value !== 'string' || !value.trim()) {
        ...
        if (manifest['note-sections']) {
            Object.entries(manifest['note-sections']).forEach(([key, value]) => {
                if (value === null) {
                    return;
                }
            ...
            if (hasWebLaunchToken && tokenConfig) {
                ...
                if (tokenConfig['use-client-authentication'] === 'no') {
                    if (!Array.isArray(tokenConfig.issuer) || tokenConfig.issuer.length === 0) {
                        ...
                        tokenConfig.issuer.forEach((issuerField, issuerIndex) => {
        ...
        if (existsSync(publisherPath)) {
            ...
            try {
                ...
                const publisherConfig = JSON.parse(publisherContent);
                const publisherErrors = [];
                // Basic publisher validation
                if (!publisherConfig.publisherId)
                    publisherErrors.push('publisherId: Field is required');
                if (!publisherConfig.publisherName)
                    publisherErrors.push('publisherName: Field is required');
                if (!publisherConfig.websiteUrl)
                    publisherErrors.push('websiteUrl: Field is required');
                if (!publisherConfig.privacyPolicyUrl)
                    publisherErrors.push('privacyPolicyUrl: Field is required');
                if (!publisherConfig.supportUrl)
                    publisherErrors.push('supportUrl: Field is required');
                if (!publisherConfig.contactEmail)
                    publisherErrors.push('contactEmail: Field is required');
                // URL validation
                const urlFields = ['websiteUrl', 'privacyPolicyUrl', 'supportUrl'];
                urlFields.forEach(field => {
                    const url = publisherConfig[field];
                    if (url) {
                        try {
                            new URL(url);
                        }
