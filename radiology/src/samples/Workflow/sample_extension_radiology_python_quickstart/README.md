# Partner Extension Sample — Radiology Quickstart (Python)

A minimal radiology extension for Dragon Copilot, written in **Python** with
...
[C# Quickstart](../SampleExtension.Radiology.Web.Quickstart/README.md).
...
Radiology sample, so don't run both on the same host without changing one.)
...
`radiology/src/samples/Workflow` directory.
...
From the sample root (`sample_extension_radiology_python_quickstart`), after
...
See [`radiology-extensibility-api.yaml`](../../../radiology-extensibility-api.yaml)
